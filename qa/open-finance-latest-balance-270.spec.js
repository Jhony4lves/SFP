...
test('#270 MeuPluggy atualizado: botão consulta saldo em tempo real e troca 532,22 por 274,82',async({page})=>{
  await page.addInitScript(()=>{
    ...
    const payload=()=>({
      ...
      items:[{
        ...
        institution:'Itaú',
        status:'UPDATED',
        updatedAt:window.__sfp270.updatedAt,
        accounts:[{
          ...
          balance:window.__sfp270.balance,
          updatedAt:window.__sfp270.updatedAt,
          currencyCode:'BRL',
    ...
    Object.defineProperty(window,'PluggyBridge',{configurable:true,value:{
      ...
      previewData:()=>{window.__sfp270.previewCalls++;return JSON.stringify(payload());},
      refreshBankBalances:()=>{
        window.__sfp270.refreshCalls++;
        window.__sfp270.balance=274.82;
        window.__sfp270.updatedAt='2026-09-23T12:20:00.000Z';
        return JSON.stringify({
          ...
          unavailable:0,
          failed:0,
          latestUpdateAt:'2026-09-23T12:20:00.000Z',
          accounts:[{account:1,ok:true,status:200,updateDateTime:'2026-09-23T12:20:00.000Z'}]
        });
      },
    ...
    Object.defineProperty(window,'PluggyRefreshBridge',{configurable:true,value:{
      refreshItems:()=>JSON.stringify({
        ...
        items:[{accepted:false,code:'REFRESH_PROVIDER_MANAGED',providerMessage:'Atualização automática gerenciada pelo MeuPluggy.'}]
      }),
  ...
  expect(result.diagnostic.balanceRefresh).toMatchObject({
    ok:true,
    requested:1,
    refreshed:1,
    rateLimited:0,
  ...
  expect(result.account.reconciled).toMatchObject({
    balance:274.82,
    source:'open-finance',
    providerUpdatedAt:'2026-09-23T12:20:00.000Z'
  });
  ...
  await expect(page.locator('#openFinancePreview')).toContainText('Saldo em tempo real consultado em 1 conta(s) e aplicado ao SFP');
});
...
test('#270 snapshot antigo não sobrescreve snapshot mais recente da mesma conta',async({page})=>{
  ...
  expect(result.initial).toBe(274.82);
  expect(result.balanceDate).toBe('2026-09-23');
      ...
      items:[
        {
          ...
          connectorName:'MeuPluggy',
          updatedAt:'2026-09-22T18:00:00.000Z',
          accounts:[{id:'acc-old',type:'BANK',name:'Itaú',marketingName:'Itaú',presentationName:'Itaú',balance:532.22,updatedAt:'2026-09-22T18:00:00.000Z',transactions:[]}]
        },
        {
          id:'item-itau-novo',
          institution:'Itaú',
          connectorName:'MeuPluggy',
          updatedAt:'2026-09-23T12:20:00.000Z',
          accounts:[{id:'acc-new',type:'BANK',name:'Itaú',marketingName:'Itaú',presentationName:'Itaú',balance:274.82,updatedAt:'2026-09-23T12:20:00.000Z',transactions:[]}]
        }
      ]
    });
    return{
