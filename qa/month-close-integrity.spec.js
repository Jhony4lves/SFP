...
test('MONTH-01/02/03/04 fechamento é histórico e só muda por substituição explícita', async ({ page }) => {
  ...
  await page.evaluate(async () => { document.querySelector('#closeMonth').click(); await new Promise(r => setTimeout(r, 100)); });
  ...
  await page.reload(); expect(await page.evaluate(() => state.snapshots[0])).toEqual(original);
  await page.evaluate(async () => { window.confirm = () => true; document.querySelector('#closeMonth').click(); await new Promise(r => setTimeout(r, 100)); });
  expect(await page.evaluate(() => state.snapshots[0])).toMatchObject({ income: 900, assets: 2600, debts: 100, netWorth: 2500 });
});
