...
function stateFromScreens(){
  const value=fixture('Bank truth v3');
  ...
  value.invoices=[
    {id:9002,cardId:2,month:'2026-09',status:'open',paidAmount:0,payments:[],documentDueDate:'2026-09-21',documentCloseDate:'2026-09-12'}
  ];
...
async function installBridge(page){
  await page.addInitScript(()=>{
    const payload={
      ok:true,provider:'pluggy-personal',readOnly:true,itemCount:2,accountCount:2,transactionPreviewCount:6,billCount:0,
      items:[
        {id:'nu-item',connectorName:'MeuPluggy',institution:'Nubank',status:'UPDATED',accounts:[{
          ...
          transactions:[
            {id:'nu-a',date:'2026-09-01T12:00:00.000Z',description:'Compra A',amount:100,type:'DEBIT',status:'POSTED',currencyCode:'BRL'},
            {id:'nu-b',date:'2026-09-08T12:00:00.000Z',description:'Compra B',amount:70.84,type:'DEBIT',status:'POSTED',currencyCode:'BRL'},
        ...
        {id:'it-item',connectorName:'MeuPluggy',institution:'Itaú',status:'UPDATED',accounts:[{
          ...
          creditData:{creditLimit:2090,availableCreditLimit:595.63},
          transactions:[
            {id:'it-a',date:'2026-09-01T12:00:00.000Z',description:'Compra C',amount:200,type:'DEBIT',status:'POSTED',currencyCode:'BRL'},
            {id:'it-b',date:'2026-09-11T12:00:00.000Z',description:'Compra D',amount:127.59,type:'DEBIT',status:'POSTED',currencyCode:'BRL'}
          ],bills:[]
        }]}
      ]
    };
...
async function boot(page){
  ...
  await writeIndexedDB(page,stateFromScreens());
  await page.evaluate(()=>localStorage.clear());
  await page.reload();
  await expectBootComplete(page,expect,'Bank truth v3');
  await page.waitForFunction(()=>Number(window.SFPOpenFinanceBankTruth?.version)>=3);
  await page.evaluate(async()=>{await SFPOpenFinancePersonal.preview();setPage('cartoes');renderAll();});
}

test('sem billId usa transações confirmadas do ciclo e não o total local',async({page})=>{
  await boot(page);
  const cards=page.locator('#cardsGrid .management-card--interactive');
  const nubank=cards.nth(0),itau=cards.nth(1);
  ...
  expect(truth.nubank.source).toBe('open-finance-posted-cycle');
  expect(truth.itau.amount).toBe(327.59);
  expect(truth.itau.source).toBe('open-finance-posted-cycle');
});

test('pagamento e compra pendentes não contaminam a fatura fechada',async({page})=>{
  await boot(page);
