...
test('ERR-027 regra aprendida mostra padrão, efeito, origem e pode ser editada sem reescrever histórico', async ({ page }) => {
  ...
  value.classificationRules = [{
    ...
    example: 'Mercado Central Loja 123'
  }];
  value.transactions.push({
    id: 301, kind: 'expense', desc: 'Mercado Central Loja 123', amount: 42.50,
    date: '2026-08-20', category: 'Alimentação', accountId: 1, status: 'paid', balanceImpact: true
  });
  ...
  await page.locator('#ruleEditCategory').selectOption('Trabalho');
  ...
  const result = await page.evaluate(() => ({
    rule: state.classificationRules[0],
    matched: ruleFor('SALARIO ACME AGOSTO'),
    tx: state.transactions.find(t => t.id === 301)
  }));
  expect(result.rule).toMatchObject({
    pattern: 'Salario ACME', action: 'income', category: 'Trabalho', editedByUser: true
  });
  expect(result.rule.updatedAt).toBeTruthy();
  expect(result.matched).toMatchObject({ action: 'income', category: 'Trabalho' });
