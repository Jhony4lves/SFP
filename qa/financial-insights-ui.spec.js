...
const cashflowRisk={
  ...
  evidence:{days:30,minBalanceCents:-12000,minDate:'2026-09-12',events:[]},
  actions:[]
};

const duplicate={
  id:'duplicate-candidate-1-2',
  type:'duplicate_candidate',
  severity:'warning',
  confidence:1,
  title:'Possível lançamento duplicado',
  message:'Duas movimentações idênticas de 18.98 foram encontradas em 2026-08-20.',
  explanation:'Mesma conta, data, natureza, valor e descrição normalizada.',
  evidence:{firstId:1,secondId:2,accountId:10,date:'2026-08-20',kind:'expense',amountCents:1898,desc:'Mercado Central'},
...
test.describe('Financial Insights UI',()=>{
  test('carrega módulo e monta painel na tela Hoje',async({page})=>{
    ...
    const info=await page.evaluate(()=>({
      hasRenderer:typeof window.renderFinancialInsights==='function',
      panel:!!document.querySelector('#financialInsightsPanel'),
      heading:document.querySelector('#financialInsightsTitle')?.textContent||''
    }));
    expect(info.hasRenderer).toBe(true);
    expect(info.panel).toBe(true);
    expect(info.heading).toBe('O que merece atenção');
  });

  test('renderiza severidade, evidência e explicação sem mutar o relatório',async({page})=>{
    await boot(page);
    const input=reportWith([cashflowRisk,duplicate]);
    const before=JSON.stringify(input);
    await page.evaluate(report=>{
      window.__qaInsightReport=report;
      window.financialIntelligenceSnapshot=()=>window.__qaInsightReport;
      window.renderFinancialInsights();
      window.applyPrivacy();
    },input);

    const cards=page.locator('#financialInsightsPanel .financial-insight');
    await expect(cards).toHaveCount(2);
    await expect(cards.first()).toHaveAttribute('data-severity','critical');
    await expect(cards.first()).toContainText('Risco de saldo negativo');
    await expect(cards.first().locator('.financial-insight-evidence')).toContainText('R$');
    await cards.first().locator('summary').click();
    await expect(cards.first().locator('.financial-insight-details p')).toContainText('Saldo atual');
    const after=await page.evaluate(()=>JSON.stringify(window.__qaInsightReport));
  ...
  test('ação contextual de duplicata leva para Lançamentos',async({page})=>{
    ...
    await page.evaluate(report=>{
      ...
      window.renderFinancialInsights();
    },reportWith([duplicate]));
    await page.getByRole('button',{name:'Revisar lançamentos'}).click();
  ...
  test('privacidade cobre mensagens e evidências monetárias do painel',async({page})=>{
    ...
    const message=page.locator('#financialInsightsPanel .financial-insight-copy p[data-money]');
    const evidence=page.locator('#financialInsightsPanel .financial-insight-evidence[data-money]');
    await expect(message).toHaveClass(/private-value/);
    await expect(evidence).toHaveClass(/private-value/);
  });
  ...
  test('estado sem alertas comunica cenário saudável sem inventar sinal',async({page})=>{
    ...
    const panel=page.locator('#financialInsightsPanel');
    await expect(panel).toContainText('Nenhum sinal relevante agora');
    await expect(panel.locator('.financial-insight')).toHaveCount(0);
  });
});
