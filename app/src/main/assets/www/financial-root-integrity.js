(function installFinancialRootIntegrity(global){
  ...
  function installStatementClosingInvariant(){
    ...
    const bankBackedBalanceAtDate=(accountId,closingDate)=>{
      ...
      (state.transfers||[]).filter(t=>t.statementKey||(t.statementKeys||[]).length).forEach(t=>{
        ...
        if(sameId(t.fromId,accountId)){
          const date=isoDate(t.date);if((!anchor||date>anchor)&&date<=closingDate)value-=Number(t.amount)||0;
        }
        if(sameId(t.toId,accountId)){
    ...
    const guarded=async function(){
      try{
        ...
        const a=(state.accounts||[]).find(x=>sameId(x.id,accountId));
        const closingDate=isoDate(meta?.closingDate);
        ...
        console.error('SFP statement closing invariant:',error);
        return false;
      }
      return original.apply(this,arguments);
    };
    Object.defineProperty(guarded,'__sfpStatementClosingInvariant',{value:true});
    Object.defineProperty(guarded,'__sfpOriginalImportStatement',{value:original});
    importStatement=guarded;global.importStatement=guarded;
    return true;
  }
  ...
  function installTransferSnapshotInvariant(){
    ...
    if(transferCandidateAllowed.__sfpPerAccountSnapshotCandidate!==true){
      const guardedAllowed=function(row,peer,semantic){
        if(!peer||sameId(peer.accountId,row.accountId))return false;
        const a=Number(row.amount)||0,b=Number(peer.signedAmount)||0;
        if(!a||!b||a*b>=0)return false;
        if(Math.abs(Math.round(Math.abs(a)*100)-Math.round(Math.abs(b)*100))!==0)return false;
        const days=Math.abs(dateObj(row.date)-dateObj(peer.date))/86400000;
        if(days>2)return false;
        if(peer.source==='transaction'){
          const rowSignal=transferImportSignal(row.desc)||semantic?.semanticClass==='possible_transfer';
          const peerSignal=transferImportSignal(peer.desc)||peer.semanticClass==='possible_transfer';
          const rowKnown=semantic?.economicImpact==='economic'&&['income','expense'].includes(semantic?.semanticClass);
          const peerKnown=peer.economicImpact==='economic'&&['income','expense'].includes(peer.semanticClass);
          if(!rowSignal&&!peerSignal&&(rowKnown||peerKnown))return false;
        }
        return true;
      };
      Object.defineProperty(guardedAllowed,'__sfpPerAccountSnapshotCandidate',{value:true});
      transferCandidateAllowed=guardedAllowed;global.transferCandidateAllowed=guardedAllowed;
    }

    if(finalizeStatementTransferMatch.__sfpPerAccountSnapshotFinalize!==true){
      const original=finalizeStatementTransferMatch;
      const guardedFinalize=function(row){
        ...
        (transfer.statementEvidence||[]).forEach(e=>{
          ...
          impact[e.accountId]=afterAccountSnapshot(e.accountId,e.date)===true;
        });
        if(Object.keys(impact).length){
          transfer.balanceImpactByAccount=impact;
          transfer.balanceImpact=Object.values(impact).some(Boolean);
        }
        return transfer;
      };
      Object.defineProperty(guardedFinalize,'__sfpPerAccountSnapshotFinalize',{value:true});
      finalizeStatementTransferMatch=guardedFinalize;global.finalizeStatementTransferMatch=guardedFinalize;
    }
    return true;
  }
  ...
  function installDebtInstallmentIdentity(){
    ...
    const guarded=function(month){
      ...
      for(const row of rows||[]){
        const debt=row?.debt;if(!debt)continue;
        const payments=(debt.history||[]).filter(h=>h?.type==='payment');
        const explicit=new Set(payments.map(h=>Number(h.installment)).filter(n=>Number.isInteger(n)&&n>=1&&n<=Number(debt.installments||0)));
        let legacy=Math.max(0,Math.trunc(Number(debt.paidInstallments)||0)-explicit.size);
        const paid=new Set(explicit);
        for(let n=1;n<=Number(debt.installments||0)&&legacy>0;n++)if(!paid.has(n)){paid.add(n);legacy--}
        row.status=paid.has(Number(row.n))?'paid':'planned';
      }
      return rows;
    };
    Object.defineProperty(guarded,'__sfpExactInstallmentStatus',{value:true});
    debtDueForMonth=guarded;global.debtDueForMonth=guarded;
    return true;
  }

  function installFinancialSemantics(){
    let savingsReady=false,contextReady=false;
    if(typeof actualSavings==='function'){
      if(actualSavings.__sfpRealizedSavings===true)savingsReady=true;
      else{
        const guarded=function(month=state?.mesAtual){
          const today=localToday();
          return (state?.transfers||[]).filter(t=>{
            const date=isoDate(t?.settledDate||t?.date);if(!date||date>today||isoMonth(date)!==month)return false;
            const from=(state.accounts||[]).find(a=>sameId(a.id,t.fromId));
            const to=(state.accounts||[]).find(a=>sameId(a.id,t.toId));
        ...
        actualSavings=guarded;global.actualSavings=guarded;savingsReady=true;
      }
    }
    if(typeof financialContextSnapshot==='function'&&typeof cashView==='function'){
      if(financialContextSnapshot.__sfpCashRealized===true)contextReady=true;
      else{
        const original=financialContextSnapshot;
        const guarded=function(options={}){
          const base=original(options)||{};
          const ref=options.reference instanceof Date?options.reference:new Date(options.reference||Date.now());
          ...
          const list=Array.from({length:months},(_,i)=>monthAdd(endMonth,i-months+1));
          const cash=list.map(month=>cashView(month));
          const incomeCents=Math.round(cash.reduce((sum,row)=>sum+(Number(row?.income)||0),0)*100);
          const expenseCents=Math.round(cash.reduce((sum,row)=>sum+(Number(row?.expense)||0),0)*100);
        ...
        financialContextSnapshot=guarded;global.financialContextSnapshot=guarded;global.sfpFinancialContextSnapshot=guarded;
        try{sfpFinancialContextSnapshot=guarded}catch{}
        contextReady=true;
      }
    }
  ...
  const install=()=>{
    const a=installStatementClosingInvariant();
    const b=installTransferSnapshotInvariant();
    const c=installDebtInstallmentIdentity();
    const d=installFinancialSemantics();
    return a&&b&&c&&d;
  };
  let attempts=0;
  const timer=setInterval(()=>{attempts++;if(install()||attempts>=200)clearInterval(timer)},25);
  install();
