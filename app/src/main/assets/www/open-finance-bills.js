...
function bankUsage(card){const used=Number(card?.openFinanceUsedAmount),limit=Number(card?.openFinanceCreditLimit),available=Number(card?.openFinanceAvailableCreditLimit);if(!Number.isFinite(used))return null;return{used:round(used),limit:Number.isFinite(limit)?round(limit):null,available:Number.isFinite(available)?round(available):null}}
function patchCardGridTruth(){
  const summary=$('cardsOutstandingTotal')?.closest?.('.metric');
  if(summary){const label=summary.querySelector('span'),hint=summary.querySelector('small');if(label)label.textContent='Compromissos projetados';if(hint)hint.textContent='faturas atuais e futuras no SFP'}
  const nodes=[...document.querySelectorAll('#cardsGrid .management-card--interactive')],cards=global.state?.cards||[];
  nodes.forEach((node,index)=>{
    const card=cards[index],truth=bankUsage(card);if(!card||!truth)return;
    const effectiveLimit=truth.limit??Number(card.limit)||0,available=truth.available??Math.max(0,effectiveLimit-truth.used),rawPct=effectiveLimit>0?truth.used/effectiveLimit*100:0,pct=Math.max(0,Math.min(100,rawPct));
    const limitStrong=node.querySelector('.sfp-card-v2-limit strong');if(limitStrong&&effectiveLimit>0)limitStrong.textContent=money(effectiveLimit);
    for(const stat of node.querySelectorAll('.sfp-card-v2-stat')){
      const label=stat.querySelector('small'),value=stat.querySelector('strong'),text=clean(label?.textContent);
      if(text==='Limite disponível'&&value)value.textContent=money(available);
      if(text==='Total comprometido'){if(label)label.textContent='Uso atual do limite';if(value)value.textContent=money(truth.used)}
    }
    const top=node.querySelectorAll('.sfp-card-v2-progress-top span');if(top[0])top[0].textContent=`${rawPct.toFixed(1).replace('.',',')}% do limite usado`;
    const bar=node.querySelector('.progress');if(bar){bar.classList.toggle('red',rawPct>80);const fill=bar.querySelector(':scope > div');if(fill)fill.style.width=`${pct}%`}
  });
}
function wrapCardUi(){if(typeof global.renderCards!=='function')return false;if(!global.renderCards.__sfpOpenFinanceTruth){const original=global.renderCards;const wrapped=function(...args){const out=original.apply(this,args);patchCardGridTruth();return out};Object.defineProperty(wrapped,'__sfpOpenFinanceTruth',{value:true});global.renderCards=wrapped}patchCardGridTruth();return true}
function wrapHealthAlerts(){if(typeof global.healthAlerts!=='function')return false;if(global.healthAlerts.__sfpOpenFinanceTruth)return true;const original=global.healthAlerts;const wrapped=function(...args){let alerts=original.apply(this,args)||[];const cards=global.state?.cards||[];let added=false;for(const card of cards){const truth=bankUsage(card);if(!truth)continue;alerts=alerts.filter(a=>clean(a?.t)!==`${card.name}: mais de 80% do limite está comprometido.`);const limit=truth.limit??Number(card.limit)||0;if(limit>0&&truth.used/limit>.8){alerts.push({c:'yellow',t:`${card.name}: uso do limite informado pelo banco está acima de 80%.`});added=true}}if(added)alerts=alerts.filter(a=>!clean(a?.t).includes('Nenhum alerta importante'));if(!alerts.length)alerts.push({c:'green',t:'Nenhum alerta importante com os dados atuais.'});return alerts};Object.defineProperty(wrapped,'__sfpOpenFinanceTruth',{value:true});global.healthAlerts=wrapped;return true}
...
function install(){if(global[FLAG])return;if(!global.state||!wrapApi()||!wrapSave()||!wrapNoTransactionSync()||!wrapCardUi()||!wrapHealthAlerts()||!ui()){setTimeout(install,50);return}global[FLAG]=true;global.SFPOpenFinanceBills=Object.freeze({version:VERSION,apply,diag,exportDiag,getLastPreview:()=>lastPreview,patchCardGridTruth})}
