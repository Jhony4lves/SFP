(function(global){
  ...
  function ensurePanel(){
    ...
    panel.id=PANEL_ID;
    panel.className='panel safe-spend-panel';
    panel.setAttribute('aria-labelledby','safeSpendTitle');
    anchor.insertAdjacentElement('afterend',panel);
    return panel;
  }

  function buildChart(points){
    const values=points.map(p=>Number(p.balanceCents)||0);
    if(!values.length)return '<div class="safe-spend-chart"></div>';
    let min=Math.min(...values,0),max=Math.max(...values,0);
    if(min===max){min-=1;max+=1}
    const width=100,height=48,pad=3,range=max-min;
    const coord=(value,index)=>{
      const x=values.length===1?50:pad+(index/(values.length-1))*(width-pad*2);
      const y=pad+((max-value)/range)*(height-pad*2);
      return [x,y];
    };
    const coords=values.map(coord);
    const baselineY=coord(0,0)[1];
    const poly=coords.map(([x,y])=>`${x.toFixed(2)},${y.toFixed(2)}`).join(' ');
    const circles=coords.map(([x,y])=>`<circle class="point" cx="${x.toFixed(2)}" cy="${y.toFixed(2)}" r="1.6"/>`).join('');
  ...
  function renderEvents(report){
    const events=(report?.projection?.timeline||[]).filter(e=>e.type!=='opening');
    if(!events.length)return '<div class="safe-spend-event"><span class="safe-spend-event-date">—</span><span class="safe-spend-event-desc">Nenhum evento conhecido na janela</span></div>';
    return events.slice(0,10).map(event=>{
      const sign=event.type==='income'?'+':'−';
      return `<div class="safe-spend-event" data-type="${escapeHtml(event.type)}"><span class="safe-spend-event-date">${escapeHtml(datePt(event.date))}</span><span class="safe-spend-event-desc" data-money>${sign} ${money(event.amountCents)} · ${escapeHtml(event.origin||'evento')}</span><span class="safe-spend-event-balance" data-money>${money(event.balanceCents)}</span></div>`;
    }).join('')+(events.length>10?`<small class="muted">+ ${events.length-10} evento(s) na projeção.</small>`:'');
  }

  function snapshot(){
    if(typeof global.financialContextSnapshot!=='function'||!global.SFPSafeSpend?.analyze)return null;
    const core=global.financialContextSnapshot({months:3});
    return global.SFPSafeSpend.analyze({snapshot:core,projectionDays:30});
  }
  ...
  function renderSafeSpendProjection(){
    ...
    let report;
    try{report=snapshot()}catch(error){report=null}
    if(!report){
      panel.dataset.status='critical';
      panel.innerHTML='<div class="head"><div><h2 id="safeSpendTitle">Quanto posso gastar?</h2><p>Projeção local baseada nas obrigações conhecidas.</p></div></div><p class="muted">Projeção temporariamente indisponível. Seus dados não foram alterados.</p>';
      return null;
    }
    panel.dataset.status=report.status;
    const safeText=report.shortfallCents>0?'R$ 0,00':money(report.safeToSpendCents);
    const caption=report.shortfallCents>0
      ? `As obrigações conhecidas excedem o saldo disponível em ${money(report.shortfallCents)}. O SFP não considera nenhum gasto adicional seguro neste momento.`
      : `Valor livre conhecido sem consumir as obrigações já provisionadas. ${report.basis}`;
    ...
    panel.innerHTML=`
      <div class="head"><div><h2 id="safeSpendTitle">Quanto posso gastar?</h2><p>Limite seguro conhecido + trajetória dos próximos ${report.projection.days} dias.</p></div><span class="badge ${report.status==='critical'?'negative':report.status==='tight'?'warning':'positive'}">${report.status==='critical'?'Pressão de caixa':report.status==='tight'?'Margem curta':'Cobertura saudável'}</span></div>
      <div class="safe-spend-grid">
        <article class="safe-spend-hero">
          <span class="safe-spend-eyebrow">Seguro conhecido agora</span>
          <strong class="safe-spend-value" data-money>${safeText}</strong>
          <p class="safe-spend-caption" data-money>${escapeHtml(caption)}</p>
          <div class="safe-spend-equation">
            <div><small>Disponível</small><strong data-money>${money(report.availableCents)}</strong></div>
            <div><small>Reservado</small><strong data-money>${money(report.reservedCents)}</strong></div>
            <div><small>Livre</small><strong data-money>${money(report.freeCents)}</strong></div>
          </div>
          <div class="safe-spend-equation">
            <div style="grid-column:1/-1"><small>Próxima entrada conhecida</small><strong data-money>${escapeHtml(nextIncome)}</strong></div>
          </div>
        </article>
        <article class="safe-spend-projection">
          <div class="safe-spend-projection-head"><div><b>Trajetória de saldo</b><small>Após cada entrada/saída conhecida da janela.</small></div><div class="safe-spend-min"><span>Menor saldo</span><strong data-money>${money(report.projection.minBalanceCents)}</strong><span>${escapeHtml(datePt(report.projection.minDate))}</span></div></div>
          ${buildChart(report.projection.timeline)}
          <div class="safe-spend-timeline">${renderEvents(report)}</div>
        </article>
      </div>
      <div class="safe-spend-foot"><p><b>Fórmula:</b> saldo disponível − obrigações conhecidas até a próxima entrada (ou janela conservadora do core quando não há entrada prevista). Não inclui compras futuras ainda não registradas e não esconde margem arbitrária.</p><button type="button" class="ghost" id="safeSpendAskSophy">Perguntar à Sophy</button></div>`;
    const ask=panel.querySelector('#safeSpendAskSophy');
    if(ask)ask.onclick=async()=>{
      if(typeof global.setPage==='function')global.setPage('sophy');
      if(typeof global.sophySendMessage==='function')await global.sophySendMessage('Explique meu limite de gasto seguro conhecido e a projeção de caixa atual usando o Local Financial Core. Diferencie saldo disponível, reservado e livre; cite o menor saldo projetado e a próxima entrada conhecida. Não invente buffer, não recalcule por conta própria e deixe claras as limitações da projeção.');
    };
    return report;
  }

  global.safeSpendingSnapshot=snapshot;
  global.renderSafeSpendProjection=renderSafeSpendProjection;
