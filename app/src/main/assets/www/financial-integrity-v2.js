(function(global){
  ...
  function installTimeAndLiquidityCore(){
    ...
    const pending=function(days=7,reference=new Date()){const today=isoDate(reference),end=addDays(today,days);return baseEconomicEvents(days,reference).filter(e=>e.overdue||(String(e.date)>=today&&String(e.date)<=end)).filter(e=>e.type==='expense'?e.status!=='paid':true).map(e=>({...e,date:e.dueDate||e.date})).sort(eventComparator)};
  ...
  function updateTodayLabels(){
    ...
    try{
      const l=liquiditySnapshot({days:HORIZON_DAYS});
      if(l.accountRisks.length){
